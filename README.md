...
- Standalone CLI `@openai/codex` и выполненный `codex login`
...
Codex-transport встроен в плагин: `vendor/codex-companion/` содержит runtime, а `bin/codex-dispatch` резолвит его из marketplace install или dev-tree и изолирует состояние в `~/.claude/plugins/data/vassal-litigator-cc-codex`. Подробный контракт вызова описан в `skills/codex-invocation/SKILL.md`.
...
Из папки установленного плагина (Claude Code: `~/.claude/plugins/cache/strigov-cc-plugins/vassal-litigator-cc/<version>/`):
...
### 3. Установите Codex CLI и войдите в аккаунт

Codex-transport плагина уже завендорен, но для OAuth-логина и самого `codex` binary нужен standalone CLI:
...
├── bin/
│   └── codex-dispatch        # Wrapper для vendored codex-companion
├── skills/                   # Скиллы (15)
...
├── vendor/
│   └── codex-companion/      # Vendored runtime для Codex transport
...
- Резолвер Codex transport переведён на vendored runtime: основной вход `bin/codex-dispatch`, legacy `$CODEX_COMPANION` указывает на `vendor/codex-companion/scripts/codex-companion.mjs`, состояние broker изолировано в `~/.claude/plugins/data/vassal-litigator-cc-codex`.
